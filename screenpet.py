...
def toggle_eyes():
    ...
    new_color = c.body_color if current_color == 'white' else 'white'  # for blinking make eye volor equal to body color
    current_state = c.itemcget(pupil_left , 'state')
    new_state = NORMAL if current_state == HIDDEN else HIDDEN    
    c.itemconfigure(pupil_left , state = new_state)     # for blinking make pupil disappear that is hidden
...
def blink():
    toggle_eyes()
    win.after(250,toggle_eyes)    # function call after some time
